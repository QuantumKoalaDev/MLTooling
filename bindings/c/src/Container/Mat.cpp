...
extern "C"
{
    ...
    MLTOOLING_CAPI mlt_status
    mlt_ct_mat_create_with_val(unsigned long rows, unsigned long cols, float init, Mat** out_mat)
    {
        if (!out_mat)
        ...
        try
        {
            *out_mat = new Mat(Container::Mat{static_cast<size_t>(rows), static_cast<size_t>(cols), init});
        }
    ...
    MLTOOLING_CAPI mlt_status mlt_ct_mat_create_from_flat_array(
        const unsigned long rows, const unsigned long cols, const float* data, Mat** out_mat
    )
    {
        if (!out_mat)
            return MLT_ERR_NULL_PTR;
    ...
    MLTOOLING_CAPI mlt_status
    mlt_ct_mat_get_item(const unsigned long row, const unsigned long col, const Mat* mat, float* out)
    {
        if (!mat || !out)
            return MLT_ERR_NULL_PTR;
    ...
    MLTOOLING_CAPI mlt_status
    mlt_ct_mat_set_item(const unsigned long row, const unsigned long col, const float value, Mat* mat)
    {
        if (!mat)
            return MLT_ERR_NULL_PTR;
