...
Mat::Mat(const size_t rows, const size_t cols, const float init) :
    m_shape({rows, cols}), m_mat(std::vector<float>(rows * cols, init))
{
}
...
{
    if (rowIndex >= m_shape.rows) throw std::out_of_range("Row index out of bound");
...
{
    if (m_mat.size() > 0 && row.size() != m_shape.cols) throw std::invalid_argument("The given row is to big!");
...
{
    if (row >= m_shape.rows || col >= m_shape.cols) throw std::out_of_range("Index out of range");
...
Mat& Mat::operator+=(const Mat& other)
{
    ...
    for (size_t i = 0; i < n; ++i) { m_mat[i] += other.m_mat[i]; }
...
Mat Mat::operator*(const Mat& other) const
{
    if (m_shape.cols != other.m_shape.rows) throw std::invalid_argument("Shape mismatch");
